...
import {
  ...
  ElPagination,
  ElDialog,
  ElMessage, ElForm, ElFormItem, ElNotification
} from 'element-plus'
import { Search, Refresh, Wallet, Plus } from '@element-plus/icons-vue'
...
// 对话框相关处理
const currentParkingForm = ref({})
const openDialog = (row = null) => {
  dialogVisible.value = true
  if (row) {
    currentParkingForm.value = row
  }
}
const submitForm = async () => {
  if (currentParkingForm.value.id) {
    // 更新
    const res = await parkingInfoStore.updateParking(currentParkingForm.value)
    if (res.code === 200) {
    ...
    // 新增
    const res = await parkingInfoStore.addParking(currentParkingForm.value)
    if (res.code === 200) {
      ElNotification.success({
        title: '成功',
        message: res.message
...
const delParking = async (parkingId) => {
  const res = await parkingInfoStore.delParking(parkingId)
  if (res.code === 200) {
    ElNotification.success({
      title: '成功',
      message: res.message
...
<template>
  ...
  <div class="p-4 space-y-4">
    ...
    <el-card class="bg-white/80 backdrop-blur-sm border-none shadow-md">
      <template #header>
        <div class="flex items-center justify-between">
          ...
          <el-button type="primary" @click="openDialog({})" :icon="Plus">分配车位</el-button>
        </div>
      </template>
    ...
    <el-card class="bg-white/80 backdrop-blur-sm border-none shadow-md overflow-hidden">
      ...
      >
        ...
        <el-table-column prop="ownerId" label="所有人ID">
          <template #default="{ row }">
            <div class="font-mono text-sm text-blue-600">{{ row.ownerId }}</div>
          </template>
        </el-table-column>
        <!-- 状态列：使用 Tag 区分 -->
        ...
        <el-table-column label="操作" align="center">
          <template #default="{ row }">
            ...
            <el-button type="danger" size="small" @click="delParking(row.id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
  ...
  <el-dialog
    ...
    center
  >
    <el-form :model="currentParkingForm" label-width="90px">
      <el-form-item label="车位编号" prop="parkingNo">
        <el-input v-model="currentParkingForm.parkingNo" :disabled="!!currentParkingForm.id"
                  placeholder="请输入P开头的车位编号" />
      </el-form-item>
      <el-form-item label="请分配业主" prop="ownerId">
        <el-input
          v-model.number="currentParkingForm.ownerId"
